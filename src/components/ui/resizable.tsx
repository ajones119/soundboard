...
function ResizableHandle({
  ...
  return (
    ...
    >
      {withHandle && (
        <div className="bg-border z-10 flex h-3 w-4 items-center justify-center rounded-xs border group-data-[orientation=vertical]:h-4 group-data-[orientation=vertical]:w-3">
          <GripVerticalIcon className="size-2.5 group-data-[orientation=vertical]:hidden" />
          <GripHorizontalIcon className="size-2.5 hidden group-data-[orientation=vertical]:block" />
        </div>
      )}
    </Separator>
  )
